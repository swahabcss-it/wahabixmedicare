...
def staff_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if new_username and new_username != user.username and User.objects.filter(username=new_username).exclude(pk=user.pk).exists():
            ...
            return render(request, 'clinic_admin/staff_form.html', {'staff': staff, 'action': 'Edit', 'roles': ASSIGNABLE_ROLES, 'is_self': is_self, 'staff_extra_roles': staff.get_extra_roles_list()})
        if new_username:
