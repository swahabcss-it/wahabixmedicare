...
class Clinic(models.Model):
    ...
    def enabled_modules(self):
        mods = []
        if self.is_lab_enabled: mods.append(('lab', '🔬', 'Laboratory'))
        if self.is_pharmacy_enabled: mods.append(('pharmacy', '💊', 'Pharmacy'))
        if self.is_hr_enabled: mods.append(('hr', '👥', 'HR & Payroll'))
        if self.is_reception_enabled: mods.append(('reception', '🏪', 'Reception'))
        if self.is_doctor_enabled: mods.append(('doctor', '👨‍⚕️', 'Doctor'))
        if self.is_billing_enabled: mods.append(('billing', '💰', 'Billing'))
        if self.is_assets_enabled: mods.append(('assets', '🛠️', 'Assets'))
        return mods
