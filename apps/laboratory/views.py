...
from .range_utils import auto_flag_abnormal
...
def test_create(request):
    try:
        ...
        profile = None
    if not (request.user.is_superuser or (profile and (profile.role == 'clinic_admin' or profile.can_edit_lab_catalog))):
        messages.error(request, '⛔ Your Clinic Admin has not granted you access to edit the Lab Test Catalogue.')
...
def test_soft_delete(request, pk):
    """Soft delete only if this person has been granted Lab Catalogue edit permission by Clinic Admin
    (catalogue changes — price, reference range, removing a test — are gated separately from
    deleting a PATIENT's lab results, which uses can_delete_lab_results instead)."""
    try:
        profile = request.user.staff_profile
    except Exception:
        profile = None
...
def result_entry(request, order_pk):
    ...
    tests_with_results = []
    for test in order.tests.all():
        existing = existing_results.get(test.pk)
        # Suggested flag for the checkbox's initial state: prefer whatever
        # was already saved; otherwise auto-detect from a saved value if
        # one somehow exists without a flag; otherwise leave unset (None)
        # so the JS/live-typing check takes over once they start typing.
        if existing:
            suggested_abnormal = existing.is_abnormal
        else:
            suggested_abnormal = None
        tests_with_results.append({
            'test': test,
            'result': existing,
            'suggested_abnormal': suggested_abnormal,
        })
