...
from apps.core.submodules import SUBMODULE_REGISTRY, MODULE_LABELS, MODULE_CLINIC_FLAG, clinic_enabled_submodules
...
def clinic_detail(request, pk):
    ...
    if request.method == 'POST' and request.POST.get('action') == 'save_submodules':
        new_map = {}
        for module, items in SUBMODULE_REGISTRY.items():
            module_map = {key: (request.POST.get(f'sub_{module}_{key}') == 'on') for key, _label in items}
            new_map[module] = module_map
        clinic.submodule_map = new_map
        clinic.save(update_fields=['submodule_map'])
        AuditLog.log(f'Updated sub-module access for: {clinic.name}', user=request.user, clinic=clinic, request=request)
        messages.success(request, f'Sub-module configuration saved for "{clinic.name}".')
        return redirect('superadmin:clinic_detail', pk=clinic.pk)

    modules = [
        (code, MODULE_LABELS[code], getattr(clinic, flag))
        for code, flag in MODULE_CLINIC_FLAG.items()
    ]
    return render(request, 'superadmin/clinic_detail.html', {
        'clinic': clinic, 'staff': staff, 'modules': modules,
        'module_flags': {code: getattr(clinic, flag) for code, flag in MODULE_CLINIC_FLAG.items()},
        'submodule_registry': SUBMODULE_REGISTRY,
        'clinic_submodules': clinic_enabled_submodules(clinic),
    })
